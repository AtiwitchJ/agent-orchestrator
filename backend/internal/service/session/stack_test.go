...
func TestDeriveStatusWorstWinsAcrossIndependentPRs(t *testing.T) {
	...
	if got := deriveStatus(live(), prs, statusNow, statusStallThreshold, true); got != domain.StatusCIFailed {
...
func TestDeriveStatusAllMergeableReportsMergeable(t *testing.T) {
	...
	if got := deriveStatus(live(), prs, statusNow, statusStallThreshold, true); got != domain.StatusMergeable {
...
func TestDeriveStatusMergedParentOpenChildStaysOnChild(t *testing.T) {
	...
	if got := deriveStatus(live(), prs, statusNow, statusStallThreshold, true); got != domain.StatusReviewPending {
...
func TestDeriveStatusAllMergedReportsMerged(t *testing.T) {
	...
	if got := deriveStatus(live(), prs, statusNow, statusStallThreshold, true); got != domain.StatusMerged {
...
func TestDeriveStatusAllClosedNoneMergedFallsToActivity(t *testing.T) {
	...
	if got := deriveStatus(statusRec(domain.ActivityActive, false), prs, statusNow, statusStallThreshold, true); got != domain.StatusWorking {
		t.Fatalf("got %q want working", got)
	}
}

func TestDeriveStatusEmptyPRsUsesActivity(t *testing.T) {
	if got := deriveStatus(statusRec(domain.ActivityActive, false), nil, statusNow, statusStallThreshold, true); got != domain.StatusWorking {
		t.Fatalf("got %q want working", got)
	}
}
