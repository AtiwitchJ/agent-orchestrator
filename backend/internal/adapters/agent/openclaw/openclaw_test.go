...
func TestGetLaunchCommandBuildsCrossPlatformArgv(t *testing.T) {
	plugin := &Plugin{resolvedBinary: "openclaw"}
	...
	// launch; openclaw mints its own native id, which GetRestoreCommand resumes by.
	want := []string{
		"openclaw",
		"--cwd", "/tmp/workspace",
		"--yolo",
...
func TestGetLaunchCommandMapsPermissionModes(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			want:        nil, // openclaw doesn't have granular permission modes
			notExpected: "--yolo",
		},
		{
			name:        "auto",
...
func TestGetRestoreCommand(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			agentSessionID: "openclaw-session-123",
			workspacePath:  "/tmp/workspace",
			permission:     ports.PermissionModeDefault,
			wantOk:         true,
			wantContains:   []string{"--cwd", "/tmp/workspace", "--session", "openclaw-session-123"},
		},
		{
			name:           "restore with bypass permissions",
			agentSessionID: "openclaw-session-456",
			workspacePath:  "/tmp/workspace",
			permission:     ports.PermissionModeBypassPermissions,
			wantOk:         true,
			wantContains:   []string{"--cwd", "/tmp/workspace", "--yolo", "--session", "openclaw-session-456"},
		},
		{
			name:           "no session id",
			agentSessionID: "",
...
func TestSessionInfoReturnsFalse(t *testing.T) {
	...
	info, ok, err := plugin.SessionInfo(context.Background(), ports.SessionRef{
		ID:       "session-123",
		Metadata: map[string]string{"agentSessionId": "openclaw-session-123"},
	})
	if err != nil {
		t.Fatal(err)
	}
	if ok {
		t.Fatalf("unexpected ok: got true, want false (SessionInfo is a no-op for openclaw)")
	}
...
func TestManifest(t *testing.T) {
	...
	if manifest.Name != "openclaw" {
		t.Fatalf("unexpected manifest name: got %q, want \"openclaw\"", manifest.Name)
	}
...
func TestAreHooksInstalledReturnsFalse(t *testing.T) {
	...
	if installed {
		t.Fatalf("unexpected installed status: got true, want false (hooks are no-op for openclaw)")
	}
}
